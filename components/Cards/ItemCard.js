...
export const ItemCard = ({items, leftCornerIconColor, title}) => {
  return (
    <View style={styles.bottomContainer}>
      <View style={styles.deliveryBar}>
      	...
      	<Text style={styles.deliveryBarText}>{title}</Text>
      </View>
      ...
      {items!==null && items.length>0 && items.map((item) =>
          <View key={item.id} style={{marginLeft: 20, marginVertical: 10}}>
            <View style={{flexDirection: 'row'}}>
              <Image
                style={styles.tinyLogo}
                source={
